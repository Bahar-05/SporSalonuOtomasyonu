...
public class dietcrudlist extends HttpServlet{
	@Override
			...
			try {
				...
				while(rs.next()) {
					...
					html+="<table style=\"width:650px; border:1px solid blue; text-align:center;\">"
				 	...
				 	+ "<form action=\"dietcrudsil.bhr\" method=\"post\">"
				 	+ "<input type=\"hidden\" name=\"id\" value=\""+id+"\">"
				 			+ "<input type=\"submit\" value=\"Sil\">"
					+ "</form>"
					+ "</td><td>"
					+ "<input type=\"button\" value=\"Guncelle\" onclick=\"update("+id+",'"+Sabah+"','"+Ogle+"','"+Aksam+"');\"></td></tr></table>";
				}				
				c.close();
